...
&printCredInfoTesting($PH_ID, 'RBS', $disp_cred, $disp_manage, $alert);
...
sub update_employee {
  ...
  $DBNAME   = $DBNAMES{"$dbin"};
  $TABLE    = $DBTABN{"$dbin"};
my  $ekey = "One if by land and two if by sea";
...
sub do_report {
  ...
  my $title = "Outcomes Credentialing - Monthly Employee Verification";
