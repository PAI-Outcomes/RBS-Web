...
$ntitle = "Medicare Part D Information";
...
sub displayMPD {
  ...
  foreach $MPD (sort @medicaredocs) {
    ...
    $webpath = qq#/members/WebShare/$MPD#;
    $dskpath = "D:/WWW/${www}.pharmassess.com/members/Webshare/$MPD";
    $dirspath = 

    (@dir) = &readdirs("$dskpath/Plan Information");
    print qq#<tr><td colspan=2><h2>Plan Information</h2></td></tr>\n#;
    foreach $dir (@dir) {
      print qq#<tr><th nowrap align=left colspan=2>\n#;
      print qq#<a href="MedicarePartD_By_Year.cgi?year=$dir">#;
      print qq#$dir Plan Information</a></th></tr>\n#;
    }

    my $cnt = 0;
    (@files) = &readfiles("$dskpath/Documents");
    ...
    foreach $filename ( sort { $yhash{$b} <=> $yhash{$a} || $fhash{$a} cmp $fhash{$b} } keys %fhash ) {
       ...
       print qq#<td nowrap><p><a href="${webpath}/Documents/${filename}?v=${today}"><strong>$prog</strong> ($ext)</a></p></td>#;
       print qq#</tr>\n#;
...
sub process_notifications {
   ...
   my $filename = "D:/WWW/${www}.pharmassess.com/members/Webshare/MedicarePartD/notification/MedD_Notification.txt";
