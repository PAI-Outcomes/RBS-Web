...
include_once './includes/ds_direct_config.php';
...
if ($result = mysqli_query($mysqli, "SELECT 
				  ...
				  rbs_340b_optin,
			       ...
			       WHERE id IN (SELECT enrollment_id
					      FROM pharmassess.direct_enrollment_detail
					     WHERE direct_enrollment_id = $direct_id AND anchor = 'Y')")) {
...
$pull['Primary_Contact_Name'] = 'Sample Name';
$pull['Primary_Contact_Email'] = 'bprowell@tdsclinical.com';
#$pull['Primary_Contact_Email'] = 'aschaeuble@tdsclinical.com';
...
$input['Anchor_Amount'] = $amount1;
$input['Pharmacy_Amount'] = $amount2;
...
try {
  ...
  if ($stmt = $mysqli->prepare("
  		UPDATE pharmassess.enrollment a, pharmassess.direct_enrollment_detail b SET
		a.rbs_enrollment_complete = ?, 
		a.rbs_fee = CASE 
                              WHEN b.anchor = 'Y' THEN ?
			      ELSE ?
                            END
		WHERE a.id = b.enrollment_id
                  AND b.direct_enrollment_id = ?
		;")) {
		$stmt->bind_param('siii', $rbs_enrollment_complete, $amount1, $amount2, $direct_id);
		$stmt->execute();
		$stmt->close();
	}
	else {
  ...
  if ($result = mysqli_query($mysqli,"SELECT * FROM $dbase.form_dtl WHERE form_mst_id = $form_id AND form_page = $pg;")) {
	...
	$y = intval($dtl{'y_cord'});
	  ...
	  elseif ( $tab_type == 'Checkbox' ) {
          ...
          if ( $tab_type == 'SignHere' ) {
            array_push($GLOBALS['RBS_SignHere'], new DocuSign\eSign\Model\SignHere([ # DocuSign SignHere field/tab
                       'document_id' => '1', 'page_number' => $GLOBALS['pageno'], 'recipient_id' => '1',
                       'tab_label' => "$val", 'x_position' => $x, 'y_position' => $y
                      ]));
          }
  	  elseif ( $tab_type == 'DateSigned' ) {
            array_push($GLOBALS['RBS_DateSigned'], new DocuSign\eSign\Model\DateSigned([ # DocuSign SignHere field/tab
                       'document_id' => '1', 'page_number' => $GLOBALS['pageno'], 'recipient_id' => '1',
                       'tab_label' => "$val", 'x_position' => $x, 'y_position' => $y
                      ]));
          }
