...
$ntitle = "Most Profitable Product Report for $Pharmacy_Names{$PH_ID}";
...
sub getData {
  ...
  foreach $key (sort keys %$RxNumbersref) {
    ...
    if ($DaySupply == 0) {
      ...
      $AverageRGMDS{$NDC} = $AverageRGM{$NDC}/$DaySupply;
    }
...
sub buildJQW {
  ...
  foreach my $NDC (sort { $AverageRGM{$b} <=> $AverageRGM{$a} or $a cmp $b } keys %AverageRGM) {
    ...
    $totalSales   = sprintf("%.2f", $TotalSales{$NDC});
    $DrugName     = $DrugNames{$NDC};
    $DrugName     =~ s/\'/\\'/g;
...
var dataAdapter = new \$.jqx.dataAdapter(source);
            ...
            if (record.records.length != 0)
            {
                var secondSource = {
                    ...
                    dataFields: [
                        ...
                        { name: "DateFilled", type: "number" },
                        { name: "SalePrice", type: "number" },
                        { name: "RebatedCost", type: "number" },
                        { name: "RebatedGM", type: "number" },
                        { name: "Quantity", type: "number" },
                        { name: "ThirdPartyPayer", type: "number" },
                ...
                if (container != null)
                {
                        container.jqxGrid({
                            ...
                            columns: [
                                ...
                                { text: "Third Party Payer", dataField: "ThirdPartyPayer", width: 130 },
    ...
    columns: [
        ...
        { text: 'Average GM per Rx', datafield: 'ARGM',    
        ...
        { text: 'Total Rx Count', datafield: 'TRX', 
                 ...
                 width: 80 },
        { text: 'Total GM', datafield: 'RGM', width: 155 },
        { text: 'Average GM per Day Supply', datafield: 'ARGMDS', 
                 renderer: function (text, align, columnsheight) {
                   var textWithLineBreak = text.replace(' ', '<br />');
                   return '<div style="margin-top: 5px; margin-left: 5px;">' + textWithLineBreak + '</div>';
                 }, 
...
\$("#excelDetailExport").click(function() {
     if (nestedGrid) {
       nestedGrid.jqxGrid('exportdata', 'csv',  'Most Profitable Product Detail Table', true, null, null, '/js/jqwidgets/PHPExport/save-file.php');
     }
});

\$("#excelExport").click(function() {
    \$("#jqxgrid").jqxGrid('exportdata', 'csv', 'Most Profitable Product Table',true,null,null,'/js/jqwidgets/PHPExport/save-file.php'); 
});
