...
#  $path_DIR   = qq#members/WebShare/Reports/$Pharmacy_Name/Special/DIR#;
  ...
  #print qq#<hr>#;
  print qq#<h2 class="report_block_header">Special Reports</h2>\n#;

# DIR Fee Reports
#    print qq#
#    <select style="width: 250px; height:25px;" onChange="if(this.selectedIndex!=0)self.location=this.options[this.selectedIndex].value">
#    <option value="" SELECTED>DIR Fee</option>
#    #;
#    opendir(DIR, $path_DIR) or warn $!;
#    @files = readdir(DIR);
#	@files = reverse sort @files;
#	foreach $file (@files) {
#      # Use a regular expression to ignore files beginning with a period
#      next if ($file =~ m/^\./ || $file =~ /Thumbs.db/i);
#      print qq#<option value="/$path_DIR/$file">$file</option>#;
#    }
#    closedir(DIR);
#    print qq#</select><br /><div style="height:10px;">&nbsp;</div>#;

# 340B Profitability Reports
    print qq#
    <select style="width: 250px; height:25px;" onChange="if(this.selectedIndex!=0)self.location=this.options[this.selectedIndex].value">
    <option value="" SELECTED>340B Profitability</option>
    #;
    opendir(DIR, $path_340B) or warn $!;
    @files = readdir(DIR);
	@files = reverse sort @files;
	foreach $file (@files) {
      # Use a regular expression to ignore files beginning with a period
      next if ($file =~ m/^\./ || $file =~ /Thumbs.db/i);
      print qq#<option value="/$path_340B/$file">$file</option>#;
    }
    closedir(DIR);
    print qq#</select><br /><div style="height:10px;">&nbsp;</div>#;
...
##    #;
##    opendir(DIR, $path_Other) or warn $!;
##    while (my $file = readdir(DIR)) {
##      # Use a regular expression to ignore files beginning with a period
##      next if ($file =~ m/^\./ || $file =~ /Thumbs.db/i);
##      print qq#<option value="/$path_Other/$file">$file</option>#;
##    }
##    closedir(DIR);
##    print qq#</select><br /><div style="height:10px;">&nbsp;</div>#;
