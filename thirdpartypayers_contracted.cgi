...
sub printline {
  ...
  if($which == 1) {
    $contractb = $contracts{$PH_ID}{$TPP_Pri_ID}{Base};
    $contracta = $contracts{$PH_ID}{$TPP_Pri_ID}{Addendum};
    $upload  = qq# <a href='/members/UploadContract.cgi?TPP_ID=$TPP_Pri_ID'>Upload Contract</a>#;
  }

  if ($contractb) {
    $base  =  qq#<a href='$contractb'><img src='/members/images/basecontract.png'width='32' height='32' alt='Base Contract' title='Base Contract'></a>#;
  }
  if ($contracta) {
    $addem =  qq#<a href='$contracta'><img src='/members/images/addendumcontract.png'width='32' height='32' alt='Addendum Contract' title='Addendum Contract'></a>#;
  }
  ...
  $tpps = qq#
            {
                "TPPID": $TPP_ID_1, "PBM": "$PBM", "Plan": "$plan", "BIN": "$bins", "HelpDesk": "$hd", "ACH": "$ACH", "Manual": "$manual", "Mac": "$MAC", "Enrollment": "$online","Contract": "$base$addem", "Upload": "$upload", "expanded": "false",
            #;    
  }
  else {
  if ($which == 2) {
...
sub readContracts {
  ...
  $webpath = qq#/members/WebShare/UploadedContracts/$PH_ID#;
  $dskpath = "D:/WWW/${www}.pharmassess.com/members/Webshare/UploadedContracts/$PH_ID";
  ...
  foreach $filename (sort {"\L$a" cmp "\L$b"} @files) {
     ...
     if ( $filename =~ /_/ ) {
        ($PID, $tpp_id, $date, $type, $rest) = split("_", $filename, 5);
        $contracts{$PID}{$tpp_id}{$type} = "$webpath/$filename";
     } else {
     }
  }
}
