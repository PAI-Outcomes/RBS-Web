...
foreach $key (sort keys %$RxNumbersref) {
  ...
  $GroupName      = $Groupsref->{$key};
  $CASHorTPP      = $CASHorTPPsref->{$key};
  $SALES{$key}    = $SALEsref->{$key};
  $PAI_Payer_Name = $PAI_Payer_Namesref->{$key};

  if ( $CASHorTPP =~ /CASH/i && !$BINSfound{$BinNumber} ) {
     $CASHBINS .= "$BinNumber,";
     $BINSfound{$BinNumber}++;
  }

  if ( $PAI_Payer_Name =~ /RedeemRx/i && $GroupName =~ /^RRX/i ) {
     my $jkey = "$BinNumber##$GroupName";
     $RedeemRxBINS{$jkey}++;
  } elsif ( $PAI_Payer_Name =~ /Beyond\s*Rx/i || $BinNumber == $BeyondRx ) {
     my $jkey = "$BinNumber##$GroupName";
     $RedeemRxBINS{$jkey}++;
  }
}
...
if ($jmonth == 12 && $jyear == 2018) {
   &ncpa;
}

if ($jmonth == 12 && $jyear >= 2016) {
   &RBSAverages;
}
# Inventory turns turned on for first quarter ADM 4.12.2016 code here:  && $jmonth >= 6

if ($jmonth =~ /3|6|9|12/ && ($jyear >= 2015)) {
   &inventoryTurns;
}
...
sub displayMedSync {
  ...
  my %Data_MedSync_PerPatient = ();
  my %Data_MedSync_Patients   = ();
  my %Data_MedSync_Claims     = ();

  my $syncedclaimprcnt = 0; 
  my $syncedpatntprcnt = 0;
  my $medsyncPP        = 0;
  ...
  for (my $lyear=$yearm2; $lyear<=$yearcur; $lyear++) {
    for (my $lmonth=1; $lmonth<=12; $lmonth++) {
      ...
      my $uniqueptntcnt     = $Rep_Unique_Patients{$RepKey} || 0;
      my $syncpatientcount  = $Rep_Synced_Patients{$RepKey} || 0;
      my $syncclaimcount    = $Rep_Synced_Claim_Count{$RepKey} || 0;
      my $totalclaim        = $Rep_Total_Brand{$RepKey} + $Rep_Total_Generic{$RepKey}; 

      $syncedclaimprcnt = $syncclaimcount/$totalclaim if ($syncclaimcount > 0); 
      $Data_MedSync_Claims{$DataLoadKey} = $syncedclaimprcnt * 100;

      $syncedpatntprcnt = $syncpatientcount/$uniqueptntcnt if ($uniqueptntcnt > 0);  
      $Data_MedSync_Patients{$DataLoadKey} = $syncedpatntprcnt * 100;

      $medsyncPP = $syncclaimcount/$syncpatientcount if ($syncpatientcount > 0); 
      $Data_MedSync_PerPatient{$DataLoadKey} = $medsyncPP;
    }
  }

  my $section_title = "Med Sync Claims - $Pharmacy_Name";
  ...
  $decimalplaces = 1;
  $rotation      = 0;
  $container     = "ID_Med_Sync_Claims";
  $type          = "Med Sync Claims";
  $yaxistitle    = "Percentage(%) of Claims Synced";
  $class         = "mainchart";
  $marginright   = "110";
  $legend        = "true";
  %data          = %Data_MedSync_Claims;
  &build_mainchart;
  ...
  my $section_title = "Med Sync Patients - $Pharmacy_Name";
  print qq#<div class="page">\n#;
  print qq#<div class="page_header"><h3>$section_title</h3></div>\n#;
  ...
  $decimalplaces = 1;
  $rotation      = 0;
  $container     = "ID_Med_Sync_Patients";
  $type          = "Med Sync Patients";
  $yaxistitle    = "Percentage(%) of Patients Synced";
  $class         = "mainchart";
  $marginright   = "110";
  $legend        = "true";
  %data          = %Data_MedSync_Patients;
  &build_mainchart;
  print qq#<div id="$container" class="$class"></div>\n#;
  print qq#<div style="clear:both"></div>\n#;
  ### End Graph ### ---------------------------------------------------------------------
  ...
  my $section_title = "Average Claims Per Synced Patient - $Pharmacy_Name";
  print qq#<div class="page">\n#;
  print qq#<div class="page_header"><h3>$section_title</h3></div>\n#;
  ...
  $decimalplaces = 1;
  $rotation      = 0;
  $container     = "ID_Med_Sync_PerPatients";
  $type          = "Average Claims Per Synced Patient";
  $yaxistitle    = "Claims";
  $class         = "mainchart";
  $marginright   = "110";
  $legend        = "true";
  %data          = %Data_MedSync_PerPatient;
  &build_mainchart;
  print qq#<div id="$container" class="$class"></div>\n#;
  print qq#<div style="clear:both"></div>\n#;
  ### End Graph ### ---------------------------------------------------------------------
...
sub displayBGDaySupply {
  
  ### Build Data Hashes ### -------------------------------------------------------------
  my %Data_DaySupply_Count_Brand   = ();
  my %Data_DaySupply_Count_Generic = ();
  my %Data_DaySupply_Count_Total   = ();
  ...
  for (my $lyear=$yearm2; $lyear<=$yearcur; $lyear++) {
    for (my $lmonth=1; $lmonth<=12; $lmonth++) {
      ...
      my $Brand_DaySupply   = $Rep_DaySupply_Brand{$RepKey} || 0;
      my $Generic_DaySupply = $Rep_DaySupply_Generic{$RepKey} || 0;
      my $Total_DaySupply   = ($Brand_DaySupply + $Generic_DaySupply) || 0;
      my $Total_Brand       = $Rep_Total_Brand{$RepKey} || 0;
      my $Total_Generic     = $Rep_Total_Generic{$RepKey} || 0;
      my $Total_BG          = ($Rep_Total_Generic{$RepKey} + $Rep_Total_Brand{$RepKey})  || 0;
      
      #Check data for blanks, replace with 'null' for graph display
      if ($Brand_DaySupply =~ /^\s*$/ || $Brand_DaySupply <= 0) {
        $Brand_DaySupply = "null";
      }
      if ($Generic_DaySupply =~ /^\s*$/ || $Generic_DaySupply <= 0) {
        $Generic_DaySupply = "null";
      }
      if ($Total_DaySupply =~ /^\s*$/ || $Total_DaySupply <= 0) {
        $Total_DaySupply = "null";
      }
      if ($Total_BG =~ /^\s*$/ || $Total_BG <= 0) {
        $Total_BG = "null";
      }
      
      $Data_DaySupply_Count_Brand{$DataLoadKey}   = "$Brand_DaySupply/$Total_Brand";
      $Data_DaySupply_Count_Generic{$DataLoadKey} = "$Generic_DaySupply/$Total_Generic";
      $Data_DaySupply_Count_Total{$DataLoadKey}   = "$Total_DaySupply/$Total_BG";

    }
  }

  my $section_title = "Average Days Supply Per Script - $Pharmacy_Name";
  print qq#<div class="page">\n#;
  print qq#<div class="page_header"><h3>$section_title</h3></div>\n#;
  ...
  $decimalplaces = 1;
  $rotation      = 0;
  $container     = "ID_Total_DaySupply_Count";
  $type          = "Total Average Days Supply Per Script";
  $yaxistitle    = "Days";
  $class         = "mainchart";
  $marginright   = "110";
  $legend        = "true";
  %data = %Data_DaySupply_Count_Total;
  &build_mainchart;
  print qq#<div id="$container" class="$class"></div>\n#;
  print qq#<div style="clear:both"></div>\n#;
  ### End Graph ### ---------------------------------------------------------------------
  ...
  $decimalplaces = 1;
  $rotation      = 0;
  $container     = "ID_Total_DaySupply_Count_Brand";
  $type          = "Brand";
  $yaxistitle    = "Days";
  $class         = "chartsmall_left";
  $marginright   = "30";
  $legend        = "false";
  %data          = %Data_DaySupply_Count_Brand;
  &build_mainchart;
  ...
  $decimalplaces = 1;
  $rotation      = 0;
  $container     = "ID_Total_DaySupply_Count_Generic";
  $type          = "Generic";
  $yaxistitle    = "Days";
  $class         = "chartsmall_left";
  $marginright   = "30";
  $legend        = "false";
  %data          = %Data_DaySupply_Count_Generic;
  &build_mainchart;
...
sub SalesByPayer {
  ...
  SELECT ncpdp FROM $DBNAMERM.sales_by_payer_new WHERE NCPDP = $NCPDP && Year = $jyear && Qtr_YTD = $jqtr";
  ...
  if ($sbp_data_present > 0) {
    ...
    SELECT * FROM (
      SELECT * FROM (
        SELECT Plan, 
               ...
               ROUND((( Generic_Count /(Generic_Count + Brand_Count))*100), 0) GDR_Percent,
               ...
               ROUND((((Generic_AWP - Generic_Sale) / Generic_AWP)*100), 0) as GER_Perent

          FROM $DBNAMERM.sales_by_payer_new 
         WHERE NCPDP = $NCPDP && Year = $jyear && Qtr_YTD = $jqtr
    ...
    if ($SBPNumOfRows > 0) {
      ...
      print "<tr><th>Plan</th><th>Rx Count</th><th>GDR</th><th>GER</th><th>Sales</th><th>% of Sales</th><th>Gross Margin</th><th>GM %</th><th>Sale/ Script</th><th>GM/ Script</th><th>Avg. Days Supply</th></tr>";
      while ( my @row = $getSBPdata->fetchrow_array() ) {
        my ($plan, $count, $generic_count, $gdr_percent, $sales, $percent, $gm, $gm_percent, $sale_per_script, $gm_per_script, $avg_day_supply, $ger_percent) = @row;
        ...
        print "<tr><td style=\"text-align: left;\">$plan</td><td>$count_disp</td><td>$gdr_percent%</td><td>$ger_percent%</td><td>\$$sales</td><td>$percent%</td><td>\$$gm</td><td>$gm_percent%</td><td>\$$sale_per_script</td><td>\$$gm_per_script</td><td>$avg_day_supply</td></tr>";
        $shown_count   += $count;
      ...
      ROUND(((SUM(Total_Generic) / (SUM(Total_Generic) + SUM(Total_Brand)))*100),0) as Total_GDR_Perent
      FROM ( 
      ...
      ### GER YTD Current Year
      $sql = "
      SELECT ROUND((((SUM(Generic_AWP) - SUM(Generic_Sale)) / SUM(Generic_AWP))*100), 0) as Total_GER_Perent
      FROM $DBNAMERM.sales_by_payer_new 
      WHERE NCPDP = $NCPDP
      && Year = $jyear
      && Qtr_YTD = $jqtr
      ";
      $getTotalGER = $dbx->prepare("$sql");
      $getTotalGER->execute;
      $NumOfRows = $getTotalGER->rows;
      if ($NumOfRows > 0) {
        while ( my @row = $getTotalGER->fetchrow_array() ) {
          ($total_ger_percent) = @row;
        }
      }
      $getTotalGER->finish;
      ...
      #Total Row
      print "<tr><td style=\"text-align: left;\">Totals (YTD Q$jqtr)</td><td>$total_count</td><td>$total_gdr_percent%</td><td>$total_ger_percent%</td><td>\$$total_rx_sales</td><td>$total_percent%</td><td>\$$total_gm</td><td>$total_gm_percent%</td><td>\$$total_sale_per_script</td><td>\$$total_gm_per_script</td><td>---</td></tr>";
      ...
      sum(Total_Brand + Total_Generic) as Count,
      ROUND(((SUM(Total_Generic) / (SUM(Total_Generic) + SUM(Total_Brand)))*100),0) as Total_GDR_Percent
      FROM ( 
      SELECT NCPDP, date, date_format(date, '%Y%m') as datef, Total_Brand_Revenue, Total_Generic_Revenue, Total_Brand_Cost, Total_Generic_Cost, Total_Brand, Total_Generic
      FROM $DBNAMERM.monthly 
      WHERE 
      ...
      ### GER YTD Hist
      $sql = "
      SELECT ROUND((((SUM(Generic_AWP) - SUM(Generic_Sale)) / SUM(Generic_AWP))*100), 1) as Total_GER_Perent
      FROM $DBNAMERM.sales_by_payer_new 
      WHERE NCPDP = $NCPDP
      && Year = $yearm1
      && Qtr_YTD = $jqtr
      ";
      $getTotalGERHist = $dbx->prepare("$sql");
      $getTotalGERHist->execute;
      $NumOfRows = $getTotalGERHist->rows;
      if ($NumOfRows > 0) {
        while ( my @row = $getTotalGERHist->fetchrow_array() ) {
          ($total_ger_percent_hist) = @row;
        }
      }
      $getTotalGERHist->finish;
      ...
      #Total Row - Last Year
      print "<tr><td style=\"text-align: left;\">Prior Year (Q$jqtr)</td><td>$total_count_hist</td><td>$total_gdr_percent_hist%</td><td>---</td><td>\$$total_rx_sales_hist</td><td>---</td><td>\$$total_gm_hist</td><td>$total_gm_percent_hist%</td><td>\$$total_sale_per_script_hist</td><td>\$$total_gm_per_script_hist</td><td>---</td></tr>";  
