...
sub displayReportHeader {
  $ntitle = "Inventory Turns for $Pharmacy_Names{$PH_ID}";
...
sub getInventoryTotals {
  ...
  my $sql = qq# 
    SELECT round(min(Inventory_Turns) - 10), round(max(Inventory_Turns) + 10)
      FROM $DBNAME.`$TABLE`
     WHERE 1=1
        && Pharmacy_ID = $PH_ID
        && Date >= '$year2'
  ...
  $turn_min++ if (($turn_min / 2) =~ /\./);
  $turn_max++ if (($turn_max / 2) =~ /\./);
