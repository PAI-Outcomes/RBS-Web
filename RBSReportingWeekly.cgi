...
##  print "here:$PH_ID" if($PH_ID < 10);
##  print $sql if($PH_ID < 10);
  ...
  if ( $NumOfRows > 0 ) {
    while ( my ($Pharmacy_Name, $NCPDP, $Date, $Total_Scripts, $Total_Revenue, $Total_Cost, $Pharmacy_ID) = $sthx->fetchrow_array() ) {
       next if (($Pharmacy_Status_RBSs{$Pharmacy_ID} !~ /^Active/i) && ($Pharmacy_Status_RBS_Directs{$Pharmacy_ID} !~ /^Active/i));
