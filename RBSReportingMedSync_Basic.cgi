...
$ntitle = "Med Sync Patient Report for $Pharmacy_Names{$Pharmacy_ID}";
print qq#<h1 class="rbsreporting">$ntitle</h1>\n#;

print qq#<form id="selectForm" action="RBSReportingMedSync_Basic.cgi" method="post">#;
...
my $sql = "SELECT a.NCPDP, b.Pharmacy_Name, b.Vendor, count(*) AS cnt
               FROM rbsreporting.basic_medsync_new a
               JOIN officedb.ncpdp_list b ON (a.ncpdp = b.providerid)
              GROUP BY a.NCPDP, b.Pharmacy_Name
              ORDER BY b.Pharmacy_Name, a.NCPDP";

my $sthx  = $dbx->prepare("$sql");
$sthx->execute;

while ( my ($NCPDP, $Pharmacy_Name, $Vendor) = $sthx->fetchrow_array() ) {
  if ( $NCPDP == $inNCPDP ) {
    $sel = 'SELECTED';
  }
  else {
  ...
  print qq#<option value="$NCPDP" $sel>$NCPDP - $Vendor - $Pharmacy_Name</option>\n#;
}
...
sub getData {
  ...
  my $tbl_medsync     = 'basic_medsync_new';
